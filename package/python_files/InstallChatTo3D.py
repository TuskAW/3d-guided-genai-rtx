...
def get_device_info_nvml():
    try:
        ...
        for i in range(deviceCount):
            ...
            fakePdi = int(hash(uuid) % 2**64)  # Hash the uuid to a 64-bit integer
...
def main():
    """Main function to execute Git clone, file replacement, Conda checks, package installation, Blender addon copying, and NGC login with Podman command."""
    ...
    # Step 10: Perform NGC login and run Podman command
    print("\nPerforming NGC login and running Podman command...")
    # Store the current working directory
    original_cwd = os.getcwd()
    logger.debug("Stored original CWD: %s", original_cwd)

    try:
        # Change CWD to target_dir
        os.chdir(target_dir)
        logger.info("Changed CWD to: %s", target_dir)
        ...
        try:
            ...
            print(f"Failed to retrieve NGC API key: {str(e)}")
            logger.error("Failed to retrieve NGC API key: %s", str(e))
            return

        # Resolve shell script path
        script_dir = target_dir / 'nim_llm'
        windows_script_path = script_dir / "start_llama_container.sh"
        if not windows_script_path.exists():
            print(f"Shell script '{windows_script_path}' does not exist.")
            logger.error("Shell script not found: %s", windows_script_path)
            return
        ...
        with open(log_file_path, "w") as log_file:
            ...
            if process.returncode != 0:
                print(f"Podman command failed with exit code {process.returncode}")
                logger.error(f"Podman command exited with code {process.returncode}")
                return

        print("Podman command executed successfully.")
        logger.info("Podman command executed successfully.")
        ...
        print(f"Unexpected error during NGC login or Podman execution: {str(e)}")
        logger.exception("Unexpected error during NGC login or Podman execution")
...
def install_trellis_pip_requirements(target_dir: Path) -> bool:
    ...
    # Ensure pynvml and requests are installed for run_ngc_podman.py
    standalone_dependencies = ['pynvml', 'requests']
    ...
    for sd in standalone_dependencies:
        ...
        try:
            ...
            print(f"Successfully installed dependency '{sd}'.")
            logger.info("Pip install output for %s: %s", sd, result.stdout.strip())
        except subprocess.CalledProcessError as e:
            print(f"Error installing dependency '{sd}': {e.stderr}")
            logger.error("Failed to install dependency %s: %s", sd, e.stderr)
            all_success = False
...
if __name__ == "__main__":
    main()
        ...
        try:
            ...
            logger.info("Restored CWD to: %s", original_cwd)
        except Exception as e:
            print(f"Error restoring original CWD '{original_cwd}': {str(e)}")
            logger.error("Failed to restore CWD: %s", str(e))
            return
