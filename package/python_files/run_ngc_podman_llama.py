...
file_handler = logging.FileHandler("run_ngc_podman.log")
...
def stop_container():
    """Attempt to stop the CHAT_TO_3D container and verify it's stopped."""
    logger.debug("Attempting to stop CHAT_TO_3D container")
    try:
        # Stop the container
        stop_process = subprocess.run(
            ["wsl", "-d", "NVIDIA-Workbench", "podman", "stop", "CHAT_TO_3D"],
            capture_output=True,
            text=True,
            timeout=30
        )
        if stop_process.returncode == 0:
            logger.info("Successfully stopped CHAT_TO_3D container.")
            print("Successfully stopped CHAT_TO_3D container.")
        else:
        ...
        check_process = subprocess.run(
            ["wsl", "-d", "NVIDIA-Workbench", "podman", "ps", "-a", "--filter", "name=CHAT_TO_3D"],
            capture_output=True,
            text=True,
            timeout=10
        )
        if "CHAT_TO_3D" not in check_process.stdout:
            logger.debug("Verified: CHAT_TO_3D container is not running.")
        else:
            logger.warning("CHAT_TO_3D container may still be running.")
            print("Warning: CHAT_TO_3D container may still be running.")
    except subprocess.TimeoutExpired:
        logger.error("Timeout while stopping CHAT_TO_3D container.")
        print("Error: Timeout while stopping CHAT_TO_3D container.")
        raise
    except subprocess.CalledProcessError as e:
        logger.error(f"Error stopping CHAT_TO_3D container: {e.stderr}")
        print(f"Error stopping CHAT_TO_3D container: {e.stderr}")
        raise
    except Exception as e:
        logger.error(f"Unexpected error stopping CHAT_TO_3D container: {e}")
        print(f"Error: Unexpected error stopping CHAT_TO_3D container: {e}")
        raise
...
def main():
    ...
    try:
        ...
        log_file_path = script_dir / "nim_llm" / "llama_container.log"
                ...
                while True:
                    try:
                        ...
                        if time.time() - start_time > timeout_seconds:
                            logger.error("Timeout: 'Application startup complete' not detected within 5 minutes.")
                            print("Error: Timeout: 'Application startup complete' not detected within 5 minutes.")
                            stop_container()  # Attempt to clean up
                            sys.exit(1)
